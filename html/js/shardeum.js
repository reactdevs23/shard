...
function updateScales() {
  $(".has-scale").each(function () {
    if (
      ...
      $(this).find(".scale-container").attr({
        style: "",
      });
    }
  });
...
$(".cta-btn.split").each(function () {
  ...
  if ($(this).data("icon")) {
    ctaData +=
      '<img src="./html/img/' + $(this).data("icon") + '" class="cta-icon">';
  }
...
function openSideMenu() {
  ...
  gsap.set($(".side-menu").find(".menu-inner"), {
    opacity: 1,
    y: 0,
  });
...
function initTicker(el) {
  ...
  // position each group
  gsap.set(trg.find(".scroll-group"), {
    width: totTickerW,
  });
  gsap.set(trg.find('.scroll-group[data-num="2"]'), {
    left: totTickerW,
  });

  // set full width of slider wrap
  gsap.set(trg.find(".scroll-mover"), {
    width: totTickerW * 2,
  });
  ...
  trg.parents(".has-ticker").attr({
    "data-ticker": t - 1,
    "data-tickerw": totTickerW,
  });
  t++;
...
function buildSwiper(el) {
  let id = el.attr("id");
  let swBoundX =
    Number(el.data("total")) * Number(el.data("modw")) -
    Number(el.data("modw"));
  ...
  Draggable.create("#" + id, {
    ...
    snap: {
      x: function (endValue) {
        let swGridW =
          Number($(this.target).data("modw")) +
          Number($(this.target).data("gap"));
...
function updateRoadmap(el) {
  ...
  if (winW <= 1024) {
    ...
    gsap.set(el.find(".rm-slide, .rm-year"), {
      width: rmModW,
    });

    // setup for full width container
    gsap.set(el.find(".slide-wrap"), {
      width: rmW,
      x: 0,
    });
...
function sendForm(formObj) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      gsap.to($(formObj).parents(".has-form").find(".thank-you"), {
        duration: 0.5,
        autoAlpha: 1,
      });
...
$(".global-form input").on("focus click", function () {
  gsap.to($(this).parents(".has-form").find(".thank-you"), {
    duration: 0.25,
    autoAlpha: 0,
  });
...
if (isHome) {
  // setup lines
  gsap.set($("#home-hero").find(".path-draw"), {
    drawSVG: "0% 0%",
  });

  // setup shapes
  gsap.set($("#home-hero").find(".background.top").find(".bgshape"), {
    scaleX: 0.5,
    scaleY: 0.5,
  });
  gsap.set($("#home-hero").find(".background.top").find(".bgshape>img"), {
    opacity: 0,
  });

  // shape groups
  gsap.set(
    $("#home-hero").find('.shape-group[data-num="1"]').find(".shape-move"),
    {
      x: -250,
      y: 500,
    }
  ...
  gsap.set(
    $("#home-hero").find('.shape-group[data-num="2"]').find(".shape-move"),
    {
      x: -600,
    }
  );
  gsap.set(
    $("#home-hero").find('.shape-group[data-num="3"]').find(".shape-move"),
    {
      y: -350,
    }
  );
  gsap.set(
    $("#home-hero").find('.shape-group[data-num="4"]').find(".shape-move"),
    {
      x: 600,
    }
  );
  ...
  // individual shapes
  gsap.set($("#home-hero").find('.bgshape[data-num="2"]'), {
    y: 230,
  });
  gsap.set($("#home-hero").find('.bgshape[data-num="3"]'), {
    x: -760,
    y: 60,
  });
  gsap.set($("#home-hero").find('.bgshape[data-num="4"]'), {
    y: -380,
  });
  gsap.set($("#home-hero").find('.bgshape[data-num="6"]'), {
    y: -240,
  });
  gsap.set($("#home-hero").find('.bgshape[data-num="7"]'), {
    y: 200,
  });
  ...
  $(shapeOrd).each(function (i) {
    ...
    if (i == 1) {
      ...
      home_intro.to(
        $("#home-hero").find(
          '.bgshape[data-num="6"]>img, .bgshape[data-num="7"]>img'
        ),
        {
          duration: 0.5,
          opacity: 1,
    ...
    if (i == 2) {
      ...
      home_intro.to(
        $("#home-hero").find(
          '.bgshape[data-num="2"]>img, .bgshape[data-num="4"]>img'
        ),
        {
          duration: 0.5,
          opacity: 1,
  ...
  $(".faq-question").each(function () {
    if ($(this).hasClass("open") && $(this) != obj) {
      ...
      $(this)
        .find(".answer")
        .css({
          height: $(this).find(".answer>div").outerHeight(),
        });
...
$(".has-open.ctablock").each(function () {
  ...
  gsap.set(el.find(".bucket-img"), {
    y: -200,
  });
  gsap.set(el.find("h2, .bot"), {
    opacity: 0,
  });
});
...
$(".big-hl-wrap").each(function () {
  ...
  gsap.set(el.find("g.top"), {
    x: 0,
    y: -100,
  });
  gsap.set(el.find("g:last-child"), {
    x: 0,
    y: 10,
  });
});
...
if (winW > 768) {
  $("#home-buckets")
    ...
    .each(function (i) {
      ...
      let bW = Number(
        $(this).outerWidth() *
          $(this).parents(".buckets").find(".bucket").length
      );
